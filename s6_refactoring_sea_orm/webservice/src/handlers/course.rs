...
use crate::dbaccess::course::{delete_course_db, get_all_courses_for_db, get_course_details_db, get_courses_for_teacher_db, post_new_course_db, update_course_details_db};
...
pub async fn get_all_course(
    ...
    get_all_courses_for_db(&app_state.db)
        .await
        .map(|courses| HttpResponse::Ok().json(R::ok(courses)))
}
...
pub async fn post_new_course(
    ...
    let r = new_course.into_inner();
    post_new_course_db(&app_state.db, r.table_name().to_string())
        .await
        .map(|course| HttpResponse::Ok().json(R::ok(course)))
}
...
pub async fn delete_course(
    ...
    delete_course_db(&app_state.db, teacher_id, course_id)
        .await
        .map(|resp| HttpResponse::Ok().json(R::ok(resp)))
}

pub async fn update_course_details(
    app_state: web::Data<AppState>,
    update_course: web::Json<course::Model>,
    params: web::Path<(i32, i32)>,
) -> Result<HttpResponse, MyError> {
    let (teacher_id, course_id) = params.into_inner();
    let m: course::Model = update_course.into_inner();
    update_course_details_db(&app_state.db, teacher_id, course_id, m)
        .await
        .map(|course| HttpResponse::Ok().json(R::ok(course)))
}
